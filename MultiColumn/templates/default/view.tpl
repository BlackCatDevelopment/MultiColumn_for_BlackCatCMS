...
{$counter=0}
<div id="cc_multicolumn_{$section_id}"{if $equalize != 0} class="cc_multicolumn_eq"{/if}>
	{foreach $columns as column}
		{if $counter == 0}<div class="cc_multicolumn_row">{/if}
		{$counter = $counter + 1}
		{if $column.content != ''}
	<div class="cc_multicolumn cc_column_{$kind}{if $counter==$kind} cc_last_column{/if}">
	...
	{/foreach}
	{if $counter < $kind && $counter != 0}</div>{/if}
	<div class="clear"></div>
</div>