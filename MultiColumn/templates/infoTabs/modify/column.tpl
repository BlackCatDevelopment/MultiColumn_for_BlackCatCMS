...
<li class="fc_border_all fc_shadow_small fc_br_all {if !$column}prevTemp{/if}" id="catMC_{if !$column}__column_id__{else}{$column.column_id}{/if}">
	...
	<form action="{$CAT_URL}/modules/cc_multicolumn/save.php" method="post" class="ajaxForm">
		...
		<p>
			<strong class="cc_In100px">Tab-Titel:</strong>
		...
		<p>
			...
			<input type="text" name="pageURL" class="cc_In200px" value="{if $column.options.pageURL}{$column.options.pageURL}{/if}"{if !$column} disabled{/if} placeholder="PageID oder URL">
		</p>
		<p>
			<strong class="cc_In100px">Button Text:</strong>
			<input type="text" name="urlText" class="cc_In300px" value="{if $column.options.urlText}{$column.options.urlText}{/if}"{if !$column} disabled{/if} placeholder="Default: Mehr erfahren ..."><br>
