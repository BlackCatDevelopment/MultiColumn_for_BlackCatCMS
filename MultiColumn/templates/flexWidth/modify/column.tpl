...
<li class="fc_border_all fc_shadow_small fc_br_all {if !$column}prevTemp{/if}" id="catMC_{if !$column}__column_id__{else}{$column.column_id}{/if}">
	...
	<form action="{$CAT_URL}/modules/cc_multicolumn/save.php" method="post">
		...
		<input type="hidden" name="entry_options" value="col_width">
		<input type="hidden" name="_cat_ajax" value="1">
		<p>
			<strong class="cc_In200px">{translate('Column width in percent')}:</strong>
			<input type="text" name="col_width" value="{if $column.options.col_width}{$column.options.col_width}{/if}"{if !$column} disabled{/if}>
		</p>
		<hr>
